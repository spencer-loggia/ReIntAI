...
if __name__=="__main__":
    ...
    if FC_AGENT:
        if DISJOINT_CRITIC:
            ...
            agent = [FCWaterworldAgent(num_nodes=4, spatial=32, channels=3, input_channels=2, device="cpu", decode_node=None) for _ in range(NUM_BASE)]
            ...
            agent = [FCWaterworldAgent(num_nodes=4, spatial=32, channels=3, input_channels=2, device="cpu") for _ in range(NUM_BASE)]
    else:
        if DISJOINT_CRITIC:
    ...
    evo = EvoController(seed_agent=agent, epochs=EPOCHS, num_base=NUM_BASE, num_workers=8,
